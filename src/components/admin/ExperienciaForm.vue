  ...
  <div class="bg-secondary/30 backdrop-blur-md border border-border/50 rounded-xl p-6 mb-8">
    <div class="flex justify-between items-center mb-6 pb-4 border-b border-border/50">
      ...
      <div class="flex gap-2">
        <button type="button" class="px-4 py-2 rounded-lg text-sm font-medium transition-colors border" :class="currentTab === 'es' ? 'bg-primary/10 border-primary/50 text-primary' : 'bg-secondary border-border/50 text-muted-foreground hover:bg-secondary/80'" @click="setTab('es')">
          {{ $t('admin_experiencia_form.experiencia_form_1') }}
        </button>
        <button type="button" class="px-4 py-2 rounded-lg text-sm font-medium transition-colors border" :class="currentTab === 'en' ? 'bg-primary/10 border-primary/50 text-primary' : 'bg-secondary border-border/50 text-muted-foreground hover:bg-secondary/80'" @click="setTab('en')">
          {{ $t('admin_experiencia_form.experiencia_form_2') }}
        </button>
      </div>
    </div>
    ...
    <form @submit.prevent="handleSubmit" class="space-y-6">
      ...
      <div v-if="currentTab === 'en'" class="bg-gradient-to-r from-indigo-500/10 to-purple-500/10 border border-purple-500/20 rounded-xl p-4 flex justify-between items-center">
        <p class="text-sm text-muted-foreground m-0">{{ $t('admin_experiencia_form.experiencia_form_3') }}</p>
      ...
      <div v-show="currentTab === 'es'" class="space-y-4">
        <h4 class="text-base font-semibold text-foreground mb-4">{{ $t('admin_experiencia_form.experiencia_form_4') }}</h4>
        ...
        <div class="flex flex-col sm:flex-row gap-4">
          <div class="flex-1 space-y-1.5">
            ...
            <input v-model="form.link" type="url" class="w-full px-4 py-2.5 bg-background border border-border/50 rounded-lg text-sm text-foreground focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary/50" :placeholder="$t('admin_experiencia_form.experiencia_form_5')" />
          </div>
          <div class="flex-1 space-y-1.5">
            <label class="block text-sm font-medium text-muted-foreground">{{ $t('admin_experiencia_form.experiencia_form_6') }}</label>
            <input v-model="form.link_demo" type="url" class="w-full px-4 py-2.5 bg-background border border-border/50 rounded-lg text-sm text-foreground focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary/50" :placeholder="$t('admin_experiencia_form.experiencia_form_7')" />
          </div>
        </div>
        <hr class="border-t border-dashed border-border/50 my-6" />
        <h4 class="text-base font-semibold text-foreground mb-4">{{ $t('admin_experiencia_form.experiencia_form_8') }}</h4>
      </div>
      ...
      <div class="space-y-4">
        ...
        <div class="space-y-1.5">
          <label class="block text-sm font-medium text-muted-foreground">{{ $t('admin_experiencia_form.experiencia_form_9') }}</label>
        ...
        <div class="space-y-1.5">
          <label class="block text-sm font-medium text-muted-foreground">{{ $t('admin_experiencia_form.experiencia_form_10') }}</label>
          <input v-model="activeTagsInput" type="text" class="w-full px-4 py-2.5 bg-background border border-border/50 rounded-lg text-sm text-foreground focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary/50" :placeholder="$t('admin_experiencia_form.experiencia_form_11')" @blur="parseTags" />
        </div>
      </div>
